...
{
    for (size_t i = 0; i < p_amo_list_.size(); i++)
    {
...
void MainObject::HandleInputAction(SDL_Event events, Mix_Chunk* bullet_sound[2])
{
    ...
    else if (events.type == SDL_MOUSEBUTTONDOWN)
    {
        ...
        else if (events.button.button == SDL_BUTTON_RIGHT)
        {
            ...
            p_amo->LoadImg(g_name_bullet_main_fire);
            p_amo->set_type(AmoObject::SPHERE);
            p_amo->set_x_val(5);
            Mix_PlayChannel(-1, bullet_sound[1], 0);
        }

        p_amo->SetRect(this->rect_.x + this->rect_.w - 20, this->rect_.y + floor(this->rect_.h * 0.5));
...
void MainObject::HandleMove()
{
    ...
    rect_.y += y_val_;
    if (rect_.y < 0 || rect_.y + HEIGHT_MAIN_OBJECT > SCREEN_HEIGHT - 60)
    {
        rect_.y -= y_val_;
    }
}
